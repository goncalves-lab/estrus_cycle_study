...
#marker list provided in Supplementary file 1 and seurat object generated in script estrus_ov_processing.R
#################################
...
t_combined_old_ov = merge(`0002_18_mo_mm1_ov_filtered`, y = c(`0002_18_mo_mm3_ov_filtered`,`0002_18_mo_mm4_ov_filtered`,`0002_18_mo_mm5_ov_filtered`), 
                          project = "mouse_ov", merge.data=TRUE)

t_combined_old_ov <- FindVariableFeatures(t_combined_old_ov, selection.method = "vst", nfeatures = 2000)
t_combined_old_ov <- reductoR(t_combined_old_ov, 1)
save(t_combined_old_ov,file="ov_combined_18m.Rdata")
...
svm_returneR <- function(t_ge, t_old, subset_value){
  ...
  #colnames(pred.te_identity_final) <- c("identity_0.7", "identity_0.8", "identity_0.9")
  return(pred.te_identity_final)}


t_combined <- loadRData("ov_combined_new.Rdata")

t_combined_old <- loadRData("ov_combined_18m.Rdata")
...
svm_annotation_ex_lv_1 <- svm_annotation_ex_lv_1 %>% mutate(cell_labels_smv_ex = case_when(
  seurat_clusters == 0  | seurat_clusters == 1 | seurat_clusters == 2 | seurat_clusters == 3 | 
    seurat_clusters == 4 | seurat_clusters == 5 | seurat_clusters == 7 | seurat_clusters == 8 |
    seurat_clusters == 16  | seurat_clusters == 19 | seurat_clusters == 23 | seurat_clusters == 27 | seurat_clusters == 28 ~ "SC",
  seurat_clusters == 9 | seurat_clusters == 11  | seurat_clusters == 12 | seurat_clusters == 13 |
    seurat_clusters == 15  | seurat_clusters == 17 | seurat_clusters == 18 | seurat_clusters == 21 | seurat_clusters == 22 |
    seurat_clusters == 24  | seurat_clusters == 30  ~ "IC",  
  seurat_clusters == 6  | seurat_clusters == 10 | seurat_clusters == 14 | seurat_clusters == 20 | seurat_clusters == 26 | seurat_clusters == 29 ~ "EpC",
  seurat_clusters == 25  ~ "EC"
))
...
t_epc_old$labels <- pred.te_identity_lv_1_epc$identity_0.9
...
svm_annotation_ex_epc <- svm_annotation_ex_epc %>% mutate(cell_labels_smv_ex = case_when(
  seurat_clusters == 0  | seurat_clusters == 1 | seurat_clusters == 3 ~ "MGC",
  seurat_clusters ==2 | seurat_clusters == 5  | seurat_clusters == 6 | seurat_clusters == 4 |
    seurat_clusters == 7  | seurat_clusters == 9 | seurat_clusters == 14 ~ "CGC",  
  seurat_clusters == 8  | seurat_clusters == 11  ~ "LC",
  seurat_clusters == 10  | seurat_clusters == 12  ~ "EpC",
  seurat_clusters == 13 ~ "SLC"
))
...
svm_annotation_ex_sc <- svm_annotation_ex_sc %>% mutate(cell_labels_smv_ex = case_when(
  seurat_clusters == 0  | seurat_clusters == 13 | seurat_clusters == 3 | seurat_clusters == 14 ~ "ThC",
  seurat_clusters == 1 | seurat_clusters == 2  | seurat_clusters == 6 | seurat_clusters == 4 |
    seurat_clusters == 5  | seurat_clusters == 11 | seurat_clusters == 12 | seurat_clusters == 8 | seurat_clusters == 15 ~ "F",  
  seurat_clusters == 10  | seurat_clusters == 9  ~ "MC",
  seurat_clusters == 7 ~ "SC I-18M"
))
...
svm_annotation_ex_ic <- svm_annotation_ex_ic %>% mutate(cell_labels_smv_ex = case_when(
  seurat_clusters == 0  | seurat_clusters == 13 | seurat_clusters ==10| seurat_clusters == 8 ~ "MAIT",
  seurat_clusters == 4 | seurat_clusters == 19 | seurat_clusters == 9 ~ "BC",  
  seurat_clusters == 6 | seurat_clusters == 17 ~ "DC",
  seurat_clusters == 5 ~ "NKC",  
  seurat_clusters == 7 | seurat_clusters == 12~ "Mp", 
  seurat_clusters == 3 ~ "MTC",
  seurat_clusters == 16 ~ "M2Mp",
  seurat_clusters == 18 ~ "M1Mp",
  seurat_clusters == 1 | seurat_clusters == 11 | seurat_clusters == 15 | seurat_clusters == 2 ~ "iNKT",
  seurat_clusters == 14 ~ "IC I-18M",
  seurat_clusters == 20 ~ "MaC"
))
...
list[t_ec, ec_cluster, ec_subcluster] <- subseteR_part_1(t_combined_old, "EC",1)
...
save(t_combined_old,file="ov_combined_18m.Rdata")
