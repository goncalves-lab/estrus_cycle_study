...
t_combined <- merge(`0002_mouse1_od_filtered`, y = c(`0002_mouse10_od_filtered`,`0002_mouse11_od_filtered`,`0002_mouse12_od_filtered`,
                                                     `0002_mouse13_od_filtered`,`0002_mouse14_od_filtered`,`0002_mouse15_od_filtered`,
                                                     `0002_mouse16_od_filtered`,`0002_mouse2_od_filtered`,`0002_mouse3_od_filtered`,
                                                     `0002_mouse6_od_filtered`,`0002_mouse7_od_filtered`,`0002_mouse8_od_filtered`,
                                                     `0002_mouse9_od_filtered`), project = "mouse_od", merge.data=TRUE)
...
t_combined_cell_metadata <- t_combined_cell_metadata %>% mutate(condition = case_when(
  batch == "0002_mouse1_od" | batch == "0002_mouse6_od" | batch == "0002_mouse12_od" | batch == "0002_mouse13_od" ~ "proestrus",
  batch == "0002_mouse2_od" | batch == "0002_mouse8_od" | batch == "0002_mouse7_od" | batch == "0002_mouse16_od"  ~ "diestrus", 
  batch == "0002_mouse3_od" | batch == "0002_mouse9_od" | batch == "0002_mouse14_od" ~ "estrus",
  batch == "0002_mouse10_od" | batch == "0002_mouse11_od" | batch == "0002_mouse15_od" ~ "metestrus"))
...
markers <- list.load("oviduct_markers.rdata")
...
write.table(t_combined_clusters,"od_combined_clusters",sep=",",quote=F,row.names = F)
write.table(t_subset_clusters,"od_combined_clusters_subset",sep=",",quote=F,row.names = F)

new_cluster_id_t_com=c("SC", "SC", "SC", "SC", "APC", "SC","SC","?", "SC","SC","EpC","APC","SC","TC","?","?","EpC", "EpC", "NKC", "EpC", "EpC",
                       "SC", "APC", "APC", "EC", "EpC", "TC", "EpC", "BC", "APC", "APC", "?", "APC", "?", "BC")
...
t <- t %>% mutate(cell_names = case_when(
  type == 4 | type == 11 | type == 22 | type == 23  | type == 29 | type == 30 | type == 32  ~ "APC",
  type == 0 | type == 1 | type == 2 | type == 3 | type == 5 | type == 6 | type == 8 | type == 9| type == 12 | type == 21~ "SC",
  type == 10 | type == 16 | type == 17 | type ==  19 | type == 20 | type == 25 | type ==  27  ~ "EpC",
  type == 7 | type == 14 | type == 15 | type == 31 | type == 33 ~ "?",
  type == 13 | type == 26  ~ "TC",
  type == 24 ~ "EC",
  type == 34 | type == 28~ "BC",
  type == 18 ~ "NKC"
...
t_combined_cell_metadata$tissue <- "oviduct"


t_combined_cds <- garnett_annotatoR_part1(t_combined, t_combined_cell_metadata, "oviduct_markers_garnett")
t_combined <- garnett_annotatoR_part2(t_combined_cds, t_combined_cds, "oviduct_markers_garnett", t_combined)
...
garnett_annotation_ex <- garnett_annotation_ex %>% mutate(cell_labels_garnett_ex = case_when(
  seurat_clusters == 4 | seurat_clusters == 11 | seurat_clusters == 22 | seurat_clusters == 23 | seurat_clusters == 29 | seurat_clusters == 30 | seurat_clusters == 32  ~ "APC",
  seurat_clusters == 14 | seurat_clusters == 0 | seurat_clusters == 1 | seurat_clusters == 2 | seurat_clusters == 3 | seurat_clusters == 5 | seurat_clusters == 7 | 
    seurat_clusters == 6 | seurat_clusters == 8 | seurat_clusters == 9| seurat_clusters == 12  | seurat_clusters == 15 | seurat_clusters == 33~ "SC",
  seurat_clusters == 10 | seurat_clusters == 16 | seurat_clusters == 17 | seurat_clusters ==  19 | seurat_clusters == 20 
  | seurat_clusters == 25 | seurat_clusters ==  27  ~ "EpC",
  seurat_clusters == 31  | seurat_clusters == 21  ~ "Unknown",
  seurat_clusters == 13 | seurat_clusters == 26 ~ "TC",
  seurat_clusters == 24 ~ "EC",
  seurat_clusters == 34 | seurat_clusters == 28  ~ "BC",
  seurat_clusters == 18 ~ "NKC"
))
...
save(t_combined,file="od_combined_new.Rdata")

list[t_non_match_ge, t_match_ge] <- svm_subseteR(t_combined)

pred.te_identity <- svm_returneR(t_match_ge, t_non_match_ge, 0.7, 0.25)
...
svm_annotation_ex <- svm_annotation_ex %>% mutate(cell_labels_smv_ex = case_when(
  seurat_clusters == 7  | seurat_clusters == 14 | seurat_clusters == 15 | 
    seurat_clusters == 21 | seurat_clusters == 31  ~ "SC",
  seurat_clusters == 33~ "APC"
))
...
new_cluster_id_epc=c("SeC", "?", "SeC", "?", "CC", "?", "CC", "EpC", "CC", "CC", "EpC", "?", "EpC", "EpC", "CC", "?")
epc <- subseteR_part_2(t_epc, new_cluster_id_epc)

epc <- epc %>% mutate(cell_names = case_when(
  type == 0 | type == 2 ~ "SeC",
  type == 4  | type == 6 | type == 8 | type ==9 | type==14 ~ "CC",
  type == 7  | type == 10 | type == 12  | type == 13 | type == 5 ~ "EpC",
  type == 1 | type == 3  | type == 11 | type == 15 ~ "?"
...
t_epc_cell_metadata$tissue <- "oviduct"
t_combined_cds_ep <- garnett_annotatoR_part1(t_epc, t_epc_cell_metadata, "od_markers_ep")
t_epc <- garnett_annotatoR_part2(t_combined_cds_ep,t_combined_cds_ep, "od_markers_ep", t_epc)
...
garnett_annotation_ex_epc <- garnett_annotation_ex_epc %>% mutate(cell_labels_garnett_ex = case_when(
  seurat_clusters == 0 | seurat_clusters == 1 | seurat_clusters == 2 ~ "SeC",
  seurat_clusters == 4 | seurat_clusters == 6 | seurat_clusters == 8 | seurat_clusters == 9 | seurat_clusters ==  14 ~ "CC",
  seurat_clusters == 3 | seurat_clusters == 11 | seurat_clusters ==  13 | seurat_clusters == 15  ~ "Unknown",
  seurat_clusters == 5 | seurat_clusters == 7 | seurat_clusters == 10| seurat_clusters == 12 ~ "EpC"
))
...
pred.te_identity_epc <- svm_returneR(t_match_ge_epc, t_non_match_ge_epc, 0.7, 0.9)
...
svm_annotation_ex_epc <- svm_annotation_ex_epc %>% mutate(cell_labels_smv_ex = case_when(
  seurat_clusters == 11| seurat_clusters == 13 | seurat_clusters == 15 |  seurat_clusters == 3    ~ "EpC",
  seurat_clusters == 1  ~ "EpC"
))
...
save(t_epc,file="od_epc_final.Rdata")
...
new_cluster_id_sc=c("F", "F", "F", "F", "F", "F", "MC", "F","F", "F", "F", "F", "F", "F", "F", "MC", "MC", "F", "F","F", "MC", "F", "MC", "F")
sc <- subseteR_part_2(t_sc, new_cluster_id_sc)

sc <- sc %>% mutate(cell_names = case_when(
  type == 0 | type == 1 | type == 2| type == 4  | type == 8 | type == 9 | type == 11 | type == 12 | type == 14 | type == 19 | type == 21
  | type == 3 | type == 5 | type == 7 | type == 10 | type == 17 | type == 18 | type ==13  | type ==22 | type ==23~ "F",
  type == 6  | type == 15 | type == 16 | type == 20  ~ "MC"
...
t_sc_cell_metadata$tissue <- "oviduct"
t_combined_cds_ep <- garnett_annotatoR_part1(t_sc, t_sc_cell_metadata, "od_markers_sc")
t_sc <- garnett_annotatoR_part2(t_combined_cds_ep,t_combined_cds_ep, "od_markers_sc", t_sc)
...
garnett_annotation_ex_sc <- garnett_annotation_ex_sc %>% mutate(cell_labels_garnett_ex = case_when(
  seurat_clusters == 0 |  seurat_clusters == 2 | seurat_clusters == 5  | seurat_clusters ==  9 | seurat_clusters == 10 | seurat_clusters ==  11 
  | seurat_clusters == 12 | seurat_clusters ==  17 | seurat_clusters == 18  ~ "F",
  seurat_clusters == 4 | seurat_clusters == 13 | seurat_clusters ==  16 | seurat_clusters == 19  ~ "MC",
  seurat_clusters == 21 ~ "M2Mp",
  seurat_clusters == 1 |seurat_clusters == 3 | seurat_clusters == 6 | seurat_clusters == 8 | seurat_clusters == 14 | seurat_clusters == 15 
  | seurat_clusters == 20 | seurat_clusters == 7 ~ "Unknown"
))
...
svm_annotation_ex_sc <- svm_annotation_ex_sc %>% mutate(cell_labels_smv_ex = case_when(
  seurat_clusters == 1 | seurat_clusters == 3 | seurat_clusters == 6 | seurat_clusters == 7| seurat_clusters == 8 | seurat_clusters == 14 | 
    seurat_clusters == 15 | seurat_clusters == 16 | seurat_clusters == 20  ~ "F"
))
...
save(t_sc,file="od_sc_final.Rdata")
...
t_tc_cell_metadata$tissue <- "oviduct"
t_combined_cds_tc <- garnett_annotatoR_part1(t_tc, t_tc_cell_metadata, "od_markers_tc")
t_tc <- garnett_annotatoR_part2(t_combined_cds_tc,t_combined_cds_tc, "od_markers_tc", t_tc)
...
garnett_annotation_ex_tc <- garnett_annotation_ex_tc %>% mutate(cell_labels_garnett_ex = case_when(
  seurat_clusters ==  1 | seurat_clusters == 2  ~ "MAIT",
  seurat_clusters == 4 | seurat_clusters == 5 ~ "iNKT",
  seurat_clusters == 6   ~ "Unknown",
  seurat_clusters == 0 | seurat_clusters ==  3 ~ "MTC"
))
...
svm_annotation_ex_tc <- svm_annotation_ex_tc %>% mutate(cell_labels_smv_ex = case_when(
  seurat_clusters == 2   ~ "MTC",
  seurat_clusters == 6 ~ "MAIT"
))
...
save(t_tc,file="od_tc_final.Rdata")
...
new_cluster_id_apc=c("Mp", "M2Mp", "M2Mp", "M2Mp", "Mp", "?","N", "?", "M1Mp", "Mp","?","Mp","DC","DC","MaC","M1Mp", "?")
apc <- subseteR_part_2(t_apc, new_cluster_id_apc)

apc <- apc %>% mutate(cell_names = case_when(
  type == 12 | type == 13  ~ "DC",
  type == 1 | type == 2  | type == 3 ~ "M2Mp",
  type == 6~ "N",
  type == 0 | type == 4 | type == 9 | type == 11 ~ "Mp",
  type == 8 | type == 15~ "M1Mp",
  type == 5 | type == 7| type == 10 | type == 16 ~ "?",
  type == 14 ~ "MaC"
...
t_apc_cell_metadata$tissue <- "oviduct"
t_combined_cds_apc <- garnett_annotatoR_part1(t_apc, t_apc_cell_metadata, "od_markers_apc")
t_apc <- garnett_annotatoR_part2(t_combined_cds_apc,t_combined_cds_apc, "od_markers_apc", t_apc)
...
garnett_annotation_ex_apc <- garnett_annotation_ex_apc %>% mutate(cell_labels_garnett_ex = case_when(
  seurat_clusters == 7 | seurat_clusters == 13 | seurat_clusters == 15 ~ "MaC",
  seurat_clusters == 6 ~ "N",
  seurat_clusters == 1 | seurat_clusters == 2 ~ "M2Mp",
  seurat_clusters == 10 | seurat_clusters ==  12 ~ "DC",
  seurat_clusters == 0 | seurat_clusters ==11 | seurat_clusters == 8  ~ "M1Mp",
  seurat_clusters == 9 ~ "Mp",
  seurat_clusters == 3 | seurat_clusters == 5 | seurat_clusters == 4 | seurat_clusters == 14 | seurat_clusters == 16 ~ "Unknown" 
))
...
svm_annotation_ex_apc <- svm_annotation_ex_apc %>% mutate(cell_labels_smv_ex = case_when(
  seurat_clusters == 0 ~ "APC I",
  seurat_clusters == 3 |  seurat_clusters == 4 | seurat_clusters == 5 | seurat_clusters == 7 | seurat_clusters == 10 | seurat_clusters == 14
  | seurat_clusters == 15  | seurat_clusters == 13 ~ "M2Mp",
  seurat_clusters == 16 ~ "M1Mp",
  seurat_clusters == 11 ~ "Mp"
))
...
final_labels_level_2 <- rbind (final_labels_apc, final_labels_epc, final_labels_sc, final_labels_tc, final_labels_ec, final_labels_bc, final_labels_nkc)
...
level_1 <- level_1 %>% mutate(new_labels = case_when(
  old_labels == "APC" | old_labels == "TC" | old_labels == "MaC" | old_labels == "NKC" | old_labels == "BC" ~ "IC",
  old_labels == "EC" ~ "EC",
  old_labels == "EpC" ~ "EpC",
  old_labels == "SC" ~ "SC"))
...
lotR_alt("matrix/oviduct")

t_combined_alt <- merge(`0002_mouse1_od_filtered`, y = c(`0002_mouse10_od_filtered`,`0002_mouse11_od_filtered`,`0002_mouse12_od_filtered`,
                                                         `0002_mouse13_od_filtered`,`0002_mouse14_od_filtered`,`0002_mouse15_od_filtered`,
                                                         `0002_mouse16_od_filtered`,`0002_mouse2_od_filtered`,`0002_mouse3_od_filtered`,
                                                         `0002_mouse6_od_filtered`,`0002_mouse7_od_filtered`,`0002_mouse8_od_filtered`,
                                                         `0002_mouse9_od_filtered`), project = "mouse_od", merge.data=TRUE)
...
write.table(t_combined_identity,"od_combined_identity",sep=",",quote=F,row.names = F)
