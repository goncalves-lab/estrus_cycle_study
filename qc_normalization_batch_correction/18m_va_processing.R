...
#marker list provided in Supplementary file 1 and seurat object generated in script estrus_va_processing.R
#################################
...
t_combined_old_va <- merge(`0002_18_mo_mm1_va_filtered`, y = c(`0002_18_mo_mm2_va_filtered`,`0002_18_mo_mm3_va_filtered`), project = "mouse_va", merge.data=TRUE)

t_combined_old_va <- FindVariableFeatures(t_combined_old_va, selection.method = "vst", nfeatures = 2000)
t_combined_old <- reductoR(t_combined_old_va, 1)
save(t_combined_old,file="va_combined_18m.Rdata")
...
t_combined <- loadRData("va_combined_new.Rdata")
...
svm_annotation_ex_lv_1 <- svm_annotation_ex_lv_1 %>% mutate(cell_labels_smv_ex = case_when(
  seurat_clusters == 0  | seurat_clusters == 1 | seurat_clusters == 2 | seurat_clusters == 3 | 
    ...
    seurat_clusters == 9 | seurat_clusters == 10  | seurat_clusters == 11 | seurat_clusters == 14 |
    seurat_clusters == 15 | seurat_clusters == 19 | seurat_clusters == 21~ "EpC",
  seurat_clusters == 13  | seurat_clusters == 12 | seurat_clusters == 16 | seurat_clusters == 17 | seurat_clusters == 18~ "IC",  
  seurat_clusters == 4  | seurat_clusters == 20 | seurat_clusters == 22 ~ "SC"
))
...
svm_annotation_ex_epc <- svm_annotation_ex_epc %>% mutate(cell_labels_smv_ex = case_when(
  seurat_clusters == 1  | seurat_clusters == 0 | seurat_clusters == 2 | seurat_clusters == 3 | seurat_clusters == 4 |
    seurat_clusters == 5  | seurat_clusters == 6 | seurat_clusters == 7 | seurat_clusters == 8 | seurat_clusters == 9 |
    seurat_clusters == 11  | seurat_clusters == 13  ~ "BEpC",
  seurat_clusters == 10 | seurat_clusters == 12   ~ "IEpC",  
  seurat_clusters == 14  ~ "EpC I-18m-va",
  seurat_clusters == 15 ~ "Er"
))
...
svm_annotation_ex_ic <- svm_annotation_ex_ic %>% mutate(cell_labels_smv_ex = case_when(
  seurat_clusters == 3 | seurat_clusters == 4 | seurat_clusters == 7 | seurat_clusters == 8 | seurat_clusters == 10 ~ "DC",
  seurat_clusters == 12 ~ "BC",
  seurat_clusters ==9 ~ "MTC",
  seurat_clusters == 2 | seurat_clusters == 6 ~ "MAIT",
  seurat_clusters == 0 | seurat_clusters == 11 | seurat_clusters == 5 ~ "M1Mp",
