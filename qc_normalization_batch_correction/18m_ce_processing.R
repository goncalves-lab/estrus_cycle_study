...
#marker list provided in Supplementary file 1 and seurat object generated in script estrus_ce_processing.R
#################################
...
t_combined_old_ce <- merge(`0002_18_mo_mm1_ce_filtered`, y = c(`0002_18_mo_mm2_ce_filtered`,`0002_18_mo_mm3_ce_filtered`), project = "mouse_ce", merge.data=TRUE)

t_combined_old_ce <- FindVariableFeatures(t_combined_old_ce, selection.method = "vst", nfeatures = 2000)
t_combined_old_ce <- reductoR(t_combined_old_ce, 1)
save(t_combined_old_ce,file="ce_combined_18m.Rdata")
...
t_combined <- loadRData("ce_combined_new.Rdata")

t_combined_old <- loadRData("ce_combined_18m.Rdata")
...
svm_annotation_ex_lv_1 <- svm_annotation_ex_lv_1 %>% mutate(cell_labels_smv_ex = case_when(
  seurat_clusters == 15  | seurat_clusters == 17 | seurat_clusters == 18 | seurat_clusters == 19 ~ "IC",
  seurat_clusters == 0 | seurat_clusters == 2  | seurat_clusters ==1 | seurat_clusters == 3 |
    seurat_clusters == 4 | seurat_clusters == 5 | seurat_clusters == 6 | seurat_clusters == 7 | seurat_clusters == 8 |
    seurat_clusters == 9  | seurat_clusters == 10 | seurat_clusters == 11 | seurat_clusters == 12 | seurat_clusters == 14 |
    seurat_clusters == 16  | seurat_clusters == 20~ "EpC",  
...
pred.te_identity_lv_1_epc <- svm_returneR(t_ge_epc, t_ge_epc_old, 0.1)
...
svm_annotation_ex_epc <- svm_annotation_ex_epc %>% mutate(cell_labels_smv_ex = case_when(
  seurat_clusters == 0  | seurat_clusters == 1 | seurat_clusters == 2 | seurat_clusters == 3 | seurat_clusters == 4   
  | seurat_clusters == 6 | seurat_clusters == 8 | seurat_clusters == 9 | seurat_clusters == 10 | seurat_clusters == 11
  | seurat_clusters == 12 | seurat_clusters == 13 | seurat_clusters == 14 | seurat_clusters == 15 | seurat_clusters == 16~ "P/BEpC",  
  seurat_clusters == 5 | seurat_clusters == 7  ~ "CEpC"
))
...
pred.te_identity_lv_1_sc <- svm_returneR(t_ge_sc, t_ge_sc_old, 0.9)
...
svm_annotation_ex_sc <- svm_annotation_ex_sc %>% mutate(cell_labels_smv_ex = case_when(
  seurat_clusters == 1 | seurat_clusters == 2  | seurat_clusters == 0 | seurat_clusters == 3 |
    seurat_clusters == 4  | seurat_clusters == 6  ~"F",  
  seurat_clusters == 5 ~ "MC"
))
...
pred.te_identity_lv_1_ic <- svm_returneR(t_ge_ic, t_ge_ic_old, 0.3)
...
svm_annotation_ex_ic <- svm_annotation_ex_ic %>% mutate(cell_labels_smv_ex = case_when(
  seurat_clusters == 7 | seurat_clusters == 3 ~ "BC",  
  seurat_clusters == 1 | seurat_clusters == 8 ~ "DC",
  seurat_clusters == 2 ~ "MTC",
  seurat_clusters == 4 | seurat_clusters == 5 ~ "MAIT",
  seurat_clusters == 0 ~ "M1Mp",
  seurat_clusters == 6 ~ "IC I-18m-ce"
))
...
save(t_combined_old,file="ce_combined_18m.Rdata")
