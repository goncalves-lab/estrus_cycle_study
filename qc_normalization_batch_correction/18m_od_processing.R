...
#marker list provided in Supplementary file 1 and seurat object generated in script estrus_od_processing.R
#################################
...
t_combined_old_od = merge(`0002_18_mo_mm1_od_filtered`, y = c(`0002_18_mo_mm2_od_filtered`,`0002_18_mo_mm3_od_filtered`), project = "mouse_od", merge.data=TRUE)

t_combined_old_od <- FindVariableFeatures(t_combined_old_od, selection.method = "vst", nfeatures = 2000)
t_combined_old<- reductoR(t_combined_old_od, 1)
save(t_combined_old,file="od_combined_18m.Rdata")
...
t_combined <- loadRData("od_combined_new.Rdata")
...
pred.te_identity_lv_1 <- svm_returneR(t_ge_lv_1, t_old, 0.4)
...
svm_annotation_ex_lv_1 <- svm_annotation_ex_lv_1 %>% mutate(cell_labels_smv_ex = case_when(
  seurat_clusters == 0  | seurat_clusters == 4 | seurat_clusters == 5 | seurat_clusters == 20 | 
    seurat_clusters == 11 | seurat_clusters == 12  ~ "SC",
  seurat_clusters == 8  | seurat_clusters == 13 | seurat_clusters == 14 | seurat_clusters == 17 | seurat_clusters == 18 |
    seurat_clusters == 19  | seurat_clusters == 22 | seurat_clusters == 21 | seurat_clusters == 3 | seurat_clusters == 15  ~ "IC",  
  seurat_clusters == 10  | seurat_clusters == 16 | seurat_clusters == 23 |seurat_clusters == 1 | seurat_clusters == 2  | seurat_clusters ==6 | seurat_clusters == 7 | ~ "EpC",
  seurat_clusters == 13 ~ "SC"
))
...
pred.te_identity_lv_1_epc <- svm_returneR(t_ge_epc, t_ge_epc_old, 0.9)
...
svm_annotation_ex_epc <- svm_annotation_ex_epc %>% mutate(cell_labels_smv_ex = case_when(
  seurat_clusters == 1  | seurat_clusters == 2 | seurat_clusters == 8 ~ "CC",
  seurat_clusters ==0 | seurat_clusters == 3   ~ "SeC",  
  seurat_clusters == 5 | seurat_clusters == 6 |  seurat_clusters == 7 | seurat_clusters == 4   ~ "EpC"
))
...
svm_annotation_ex_sc <- svm_annotation_ex_sc %>% mutate(cell_labels_smv_ex = case_when(
  seurat_clusters == 1 | seurat_clusters == 2  | seurat_clusters == 0 | seurat_clusters == 3 |
    seurat_clusters == 4  | seurat_clusters == 5 | seurat_clusters == 6 | seurat_clusters == 8 | seurat_clusters == 7 | seurat_clusters == 10|
    seurat_clusters == 12~ "F",  
  seurat_clusters == 11  | seurat_clusters == 9  ~ "MC"
))
...
svm_annotation_ex_ic <- svm_annotation_ex_ic %>% mutate(cell_labels_smv_ex = case_when(
  seurat_clusters == 2 | seurat_clusters == 6 | seurat_clusters == 9 | seurat_clusters == 15 ~ "BC",  
  seurat_clusters == 18 | seurat_clusters == 4 | seurat_clusters == 8 ~ "DC",
  seurat_clusters == 1 | seurat_clusters == 12 | seurat_clusters == 14~ "NKC",  
  seurat_clusters == 7 | seurat_clusters == 13 | seurat_clusters == 16 | seurat_clusters == 17 ~ "MTC",
  seurat_clusters == 3 | seurat_clusters == 19 | seurat_clusters == 20 ~ "M2Mp",
  seurat_clusters == 21 ~ "M1Mp",
  seurat_clusters == 11 | seurat_clusters == 10 ~ "iNKT",
  seurat_clusters == 0 ~ "IC I-18m-od",
  seurat_clusters == 5 ~ "IC II-18m-od"
))
...
#t_combined_old <- FindVariableFeatures(t_combined_old)
list[t_ec, ec_cluster, ec_subcluster] <- subseteR_part_1(t_combined_old, "Er",1)
final_labels_ec <- data.frame("identity"="Er", "cells"=names(t_ec$seurat_clusters))
