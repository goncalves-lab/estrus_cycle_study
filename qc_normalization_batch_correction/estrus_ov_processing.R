...
t_combined <- merge(`0002_mouse1_ov_filtered`, y = c(`0002_mouse10_ov_filtered`,`0002_mouse11_ov_filtered`,`0002_mouse12_ov_filtered`,
                                                     `0002_mouse13_ov_filtered`,`0002_mouse14_ov_filtered`,`0002_mouse15_ov_filtered`,
                                                     `0002_mouse16_ov_filtered`,`0002_mouse2_ov_filtered`,`0002_mouse3_ov_filtered`,
                                                     `0002_mouse6_ov_filtered`,`0002_mouse7_ov_filtered`,`0002_mouse8_ov_filtered`,
                                                     `0002_mouse9_ov_filtered`), project = "mouse_ov", merge.data=TRUE)
...
t_combined_cell_metadata <- t_combined_cell_metadata %>% mutate(condition = case_when(
  batch == "0002_mouse1_ov" | batch == "0002_mouse6_ov" | batch == "0002_mouse12_ov" | batch == "0002_mouse13_ov" ~ "proestrus",
  batch == "0002_mouse2_ov" | batch == "0002_mouse8_ov" | batch == "0002_mouse7_ov" | batch == "0002_mouse16_ov"  ~ "diestrus", 
  batch == "0002_mouse3_ov" | batch == "0002_mouse9_ov" | batch == "0002_mouse14_ov" ~ "estrus",
  batch == "0002_mouse10_ov" | batch == "0002_mouse11_ov" | batch == "0002_mouse15_ov" ~ "metestrus"))

t_combined <- FindVariableFeatures(t_combined, selection.method = "vst", nfeatures = 2000)
t_combined <- reductoR(t_combined, 1)
...
markers <- list.load("ovary_markers.rdata")
...
write.table(t_combined_clusters,"ov_combined_clusters",sep=",",quote=F,row.names = F)
write.table(t_subset_clusters,"ov_combined_clusters_subset",sep=",",quote=F,row.names = F)

new_cluster_id_t_com=c("?", "EpC", "?", "?", "SC", "SC","APC","SC", "EpC", "?", "EpC", "EpC","SC", "EpC","EC", "EpC", "SC", "?","TC", "EpC", "TC","APC",
                       "APC" ,"EpC", "APC", "EpC", "EpC","EpC", "?")
...
t <- t %>% mutate(cell_names = case_when(
  type == 6 | type == 21 | type == 22 | type == 24  ~ "APC",
  type == 4 | type == 5 | type == 7 | type == 12 | type == 16 ~ "SC",
  type == 1 | type == 8 | type == 10 | type ==  11 | type == 13 | type == 15 | type == 19 | type ==  23 | type == 25 |
    type == 26 | type == 27  ~ "EpC",
  type == 0 | type == 2 | type == 3 | type == 9 | type == 17 | type == 28 ~ "?",
  type == 18 | type == 20  ~ "TC",
  type == 14 ~ "EC"
))
...
t_combined_cell_metadata$tissue <- "ovary"


t_combined_cds <- garnett_annotatoR_part1(t_combined, t_combined_cell_metadata, "ovary_markers_garnett")
t_combined <- garnett_annotatoR_part2(t_combined_cds, t_combined_cds, "ovary_markers_garnett", t_combined)
...
garnett_annotation_ex <- garnett_annotation_ex %>% mutate(cell_labels_garnett_ex = case_when(
  seurat_clusters == 6 | seurat_clusters == 21 | seurat_clusters == 22 | seurat_clusters == 24 ~ "APC",
  seurat_clusters == 2  | seurat_clusters == 4 | seurat_clusters == 5 | 
    seurat_clusters == 12 | seurat_clusters == 17 | seurat_clusters == 0 ~ "SC",
  seurat_clusters ==  1 | seurat_clusters == 9 | seurat_clusters == 11 | seurat_clusters ==  15 | seurat_clusters == 23 |
    seurat_clusters == 26 | seurat_clusters == 27 | seurat_clusters == 1 | seurat_clusters == 25  ~ "EpC",
  seurat_clusters == 8 | seurat_clusters == 13 | seurat_clusters == 19 | seurat_clusters == 28 | seurat_clusters == 3 | seurat_clusters == 7 
  | seurat_clusters == 10 | seurat_clusters == 16 ~ "Unknown",
  seurat_clusters == 18 | seurat_clusters == 20 ~ "TC",
...
pred.te_identity <- svm_returneR(t_match_ge, t_non_match_ge, 0.7, 0.3)
...
svm_annotation_ex <- svm_annotation_ex %>% mutate(cell_labels_smv_ex = case_when(
  seurat_clusters == 0  | seurat_clusters == 2 | seurat_clusters == 3 | 
    seurat_clusters == 7 | seurat_clusters == 16 | seurat_clusters == 17 ~ "SC",
  seurat_clusters ==  10 | seurat_clusters == 13 | seurat_clusters == 19 | seurat_clusters == 8 | seurat_clusters == 9 ~ "EpC",
...
new_cluster_id_epc=c("MGC", "MGC", "?", "LC", "CGC", "LC","MGC", "CGC", "CGC","MGC", "?" ,"CGC", "MGC", "?", "CGC","MGC", "EpC", "SLC")
epc <- subseteR_part_2(t_epc, new_cluster_id_epc)

epc <- epc %>% mutate(cell_names = case_when(
  type == 0 | type == 1  | type == 6 | type == 9 | type == 12 | type == 15 ~ "MGC",
  type == 4  | type == 7 | type == 8 | type ==11 | type==14 ~ "CGC",
  type == 3  | type == 5 ~ "LC",
  type == 16 ~ "EpC",
  type == 17 ~ "SLC",
  type == 2 | type == 10 | type == 13~ "?"
...
t_epc_cell_metadata$tissue <- "ovary"
t_combined_cds_ep <- garnett_annotatoR_part1(t_epc, t_epc_cell_metadata, "ov_markers_ep")
t_epc <- garnett_annotatoR_part2(t_combined_cds_ep,t_combined_cds_ep, "ov_markers_ep", t_epc)
...
garnett_annotation_ex_epc <- garnett_annotation_ex_epc %>% mutate(cell_labels_garnett_ex = case_when(
  seurat_clusters == 0 | seurat_clusters == 1 | seurat_clusters == 2 | seurat_clusters == 9 | seurat_clusters == 15 ~ "MGC",
  seurat_clusters == 17 ~ "SLC",
  seurat_clusters ==  3 | seurat_clusters == 5 | seurat_clusters == 6 | seurat_clusters == 12 | seurat_clusters ==  13 ~ "LC",
  seurat_clusters == 4 | seurat_clusters == 8 | seurat_clusters ==  7 | seurat_clusters == 11 | seurat_clusters == 14  ~ "CGC",
  seurat_clusters == 10  ~ "Unknown",
  seurat_clusters == 16 ~ "EpC"
))
...
pred.te_identity_epc <- svm_returneR(t_match_ge_epc, t_non_match_ge_epc, 0.7, 0.5)
...
svm_annotation_ex_epc <- svm_annotation_ex_epc %>% mutate(cell_labels_smv_ex = case_when(
  seurat_clusters == 2 ~ "CGC",
  seurat_clusters == 6| seurat_clusters == 12   ~ "MGC",
  seurat_clusters == 10 |  seurat_clusters == 13 ~ "EpC I"
))
...
save(t_epc,file="ov_epc_final.Rdata")
...
new_cluster_id_sc=c("?", "F", "ThC", "?", "?", "ThC","?", "F", "MC", "ThC", "F", "MC", "?", "?", "MC","F", "F", "?", "ThC", "MC", "F")
sc <- subseteR_part_2(t_sc, new_cluster_id_sc)

sc <- sc %>% mutate(cell_names = case_when(
  type == 5 | type == 18 | type == 9 | type == 2 ~ "ThC",
  type == 1 | type == 7  | type == 10 | type == 15 | type == 16 | type == 20 | type == 13~ "F",
  type == 8  | type == 11 | type == 14 | type == 19 ~ "MC",
  type == 0 | type == 6 | type == 12 | type == 3 | type == 4 | type == 13 | type == 17~ "?"
...
t_sc_cell_metadata$tissue <- "ovary"
t_combined_cds_ep <- garnett_annotatoR_part1(t_sc, t_sc_cell_metadata, "ov_markers_sc")
t_sc <- garnett_annotatoR_part2(t_combined_cds_ep,t_combined_cds_ep, "ov_markers_sc", t_sc)
...
garnett_annotation_ex_sc <- garnett_annotation_ex_sc %>% mutate(cell_labels_garnett_ex = case_when(
  seurat_clusters ==  1 | seurat_clusters == 5 | seurat_clusters == 6 | seurat_clusters ==  7 | seurat_clusters == 9 | 
    seurat_clusters == 10 | seurat_clusters == 13 | seurat_clusters == 15 | seurat_clusters == 16 | seurat_clusters == 20 | seurat_clusters == 4   ~ "F",
  seurat_clusters == 8 | seurat_clusters == 11 | seurat_clusters ==  14 | seurat_clusters == 19 ~ "MC",
  seurat_clusters == 0 | seurat_clusters == 3 | seurat_clusters == 12 | seurat_clusters == 17  ~ "Unknown",
  seurat_clusters == 18 | seurat_clusters == 2 ~ "ThC"
...
pred.te_identity_sc <- svm_returneR(t_match_ge_sc, t_non_match_ge_sc, 0.7, 0.5)
...
svm_annotation_ex_sc <- svm_annotation_ex_sc %>% mutate(cell_labels_smv_ex = case_when(
  seurat_clusters == 4 |  seurat_clusters == 5 | seurat_clusters == 9 | seurat_clusters == 0 | seurat_clusters == 12   ~ "ThC",
  seurat_clusters == 3 | seurat_clusters == 6 ~ "F",
  seurat_clusters == 17 ~ "SC I"
))
...
save(t_sc,file="ov_sc_final.Rdata")
...
new_cluster_id_tc=c("iNKT", "MAIT", "?", "MAIT", "MAIT", "iNKT","MAIT", "iNKT", "MTC", "MTC")
tc <- subseteR_part_2(t_tc, new_cluster_id_tc)

tc <- tc %>% mutate(cell_names = case_when(
  type == 0 | type == 5 | type == 7 ~ "iNKT",
  type == 1 | type == 3  | type == 4 | type == 6~ "MAIT",
  type == 8 | type == 9 ~ "MTC",
  type == 2~ "?"
...
t_tc_cell_metadata$tissue <- "ovary"
t_combined_cds_ep <- garnett_annotatoR_part1(t_tc, t_tc_cell_metadata, "ov_markers_tc")
t_tc <- garnett_annotatoR_part2(t_combined_cds_ep,t_combined_cds_ep, "ov_markers_tc", t_tc)
...
garnett_annotation_ex_tc <- garnett_annotation_ex_tc %>% mutate(cell_labels_garnett_ex = case_when(
  seurat_clusters ==  1 | seurat_clusters == 2 | seurat_clusters == 3 | seurat_clusters == 4 | seurat_clusters == 6 ~ "MAIT",
  seurat_clusters == 7 ~ "iNKT",
  seurat_clusters == 0 | seurat_clusters == 5   ~ "Unknown",
  seurat_clusters == 8 | seurat_clusters ==  9 ~ "MTC"
))
...
svm_annotation_ex_tc <- svm_annotation_ex_tc %>% mutate(cell_labels_smv_ex = case_when(
  seurat_clusters == 2 ~ "MAIT",
  seurat_clusters == 5   ~ "iNKT",
  seurat_clusters == 0  ~ "TC I"
))
...
save(t_tc,file="ov_tc_final.Rdata")
...
new_cluster_id_apc=c("Mp", "?", "DC", "Mp", "BC", "Mp","?", "Mp", "M1Mp", "?","Mp","DC","Mp","?","M2Mp")
apc <- subseteR_part_2(t_apc, new_cluster_id_apc)

apc <- apc %>% mutate(cell_names = case_when(
  type == 2 | type == 11 | type == 9  ~ "DC",
  type == 0 | type == 3  | type == 5 | type == 7 | type == 10 | type == 12 ~ "Mp",
  type == 4~ "BC",
  type == 8~ "M1Mp",
  type == 14 ~ "M2Mp",
  type == 1 | type == 6| type == 13 ~ "?"
...
t_apc_cell_metadata$tissue <- "ovary"
t_combined_cds_ep <- garnett_annotatoR_part1(t_apc, t_apc_cell_metadata, "ov_markers_apc")
t_apc <- garnett_annotatoR_part2(t_combined_cds_ep,t_combined_cds_ep, "ov_markers_apc", t_apc)
...
garnett_annotation_ex_apc <- garnett_annotation_ex_apc %>% mutate(cell_labels_garnett_ex = case_when(
  seurat_clusters == 3 | seurat_clusters == 5 | seurat_clusters == 6 | seurat_clusters == 7 | seurat_clusters == 12 ~ "Mp",
  seurat_clusters == 4 ~ "BC",
  seurat_clusters == 0 | seurat_clusters == 1 | seurat_clusters == 10 | seurat_clusters == 14  ~ "M2Mp",
  seurat_clusters == 2 | seurat_clusters ==  9  | seurat_clusters == 11 ~ "DC",
  seurat_clusters == 8 ~ "M1Mp",
  seurat_clusters == 13 ~ "Unknown" 
))
...
svm_annotation_ex_apc <- svm_annotation_ex_apc %>% mutate(cell_labels_smv_ex = case_when(
  seurat_clusters == 1  | seurat_clusters == 13 ~ "APC I",
  seurat_clusters == 0 |  seurat_clusters == 6 | seurat_clusters == 10 ~ "Mp"
))
...
save(t_apc,file="ov_apc_final.Rdata")
...
Idents(t_combined) <- t_combined$final_labels
t_combined <- subset(t_combined, idents = "Unknown", invert=T) 
...
final_labels_level_2 <- rbind (final_labels_apc, final_labels_epc, final_labels_sc, final_labels_tc, final_labels_ec)
...
cell_names_level_1 <- data.frame("cells"=colnames(t_combined), "level_1"=t_combined$final_labels) 

cell_names_level_1 <- cell_names_level_1 %>% mutate(level_1_cor = case_when(
  level_1 ==  "APC" | level_1 == "TC" ~ "IC",
  level_1 ==  "SC" ~ "SC",
  level_1 ==  "EC" ~ "EC",
  level_1 ==  "EpC" ~ "EpC"
))
t_combined$level_1 <- cell_names_level_1$level_1_cor
#final plots
...
ggplot(umap_plot) + 
  geom_point(aes(x = UMAP_1, y = UMAP_2, fill=level_1), shape=21, color = 'black', size = 2)+
...
ggplot(umap_plot) + 
  geom_point(aes(x = UMAP_1, y = UMAP_2, fill=level_2), shape=21, color = 'black', size = 2)+
...
lotR_alt("matrix/ovary/")

t_combined_alt <- merge(`0002_mouse1_ov_filtered`, y = c(`0002_mouse10_ov_filtered`,`0002_mouse11_ov_filtered`,`0002_mouse12_ov_filtered`,
                                                         `0002_mouse13_ov_filtered`,`0002_mouse14_ov_filtered`,`0002_mouse15_ov_filtered`,
                                                         `0002_mouse16_ov_filtered`,`0002_mouse2_ov_filtered`,`0002_mouse3_ov_filtered`,
                                                         `0002_mouse6_ov_filtered`,`0002_mouse7_ov_filtered`,`0002_mouse8_ov_filtered`,
                                                         `0002_mouse9_ov_filtered`), project = "mouse_ov", merge.data=TRUE)
...
write.table(t_combined_identity,"ov_combined_identity",sep=",",quote=F,row.names = F)

write.table(epc_cluster,"ov_epc_cluster",sep=",",quote=F,row.names = F)